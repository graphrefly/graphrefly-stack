---
name: observatory-dev-dispatch
description: Implement one approved Observatory slice from the canonical sequencer with premise checks, scoped edits, tests, evidence, and phase updates. Use when asked to implement, build, scaffold, dispatch, continue the next phase, or complete a specific OBS phase after its design decisions are locked.
---

# Observatory dev dispatch

1. Read `docs/sources.jsonl`, all locked decisions, product scope, antipatterns, and the complete
   canonical sequencer.
...
7. Apply the `observatory-qa` workflow to the resulting diff. Fix in-scope findings and rerun gates.
