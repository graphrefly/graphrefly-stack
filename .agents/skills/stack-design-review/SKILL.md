...
description: Review a GraphReFly Stack flagship scenario, commit/blueprint/domain contract, architecture, model or provider boundary, semantic change, evidence/gate shape, or synchronized Git and Blueprint UI before implementation. Use when choosing the scenario or framework, defining schemas and APIs, reviewing selective invalidation and replan, or deciding whether a design is ready for the canonical sequencer.
---
...
4. Keep claim, accepted semantic change, evidence, GateResult, and runner distinct. Also assess
   whether the user proof is understandable quickly and whether the proposal is the smallest shape
   that can satisfy the active gate.
